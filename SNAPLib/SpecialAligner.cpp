...
Routine Description:
    ...
    Constructor for the SpecialAligner class.  Aligners align reads against an indexed genome.
...
AlignmentResult SpecialAligner::CharacterizeSeeds(Read *read, unsigned searchRadius, unsigned searchLocation, bool searchRC, seed_map &map, seed_map &mapRC) {

//read, genomeLocation, hitIsRC, finalScore, searchRadius, searchLocation, searchRC, 0, NULL, NULL, NULL, NULL
    ...
    */
    ...
    if (read->getDataLength() > maxReadSize) {
        fprintf(stderr,"SpecialAligner:: got too big read (%d > %d)", read->getDataLength(),maxReadSize);
        exit(1);
    }
    ...
    while (nSeedsApplied + nRCSeedsApplied < maxSeedsToUse) {
        ...
        if (searchRadius == 0 || searchRC == false) {
            ...
            for (unsigned i = 0 ; i < min(nHits, maxHitsToConsider); i++) {           
                ...
                if ((pos == map.end())) {
                    std::set<unsigned> blah;
                    blah.insert(nextSeedToTest);
                
                    map.insert(seed_map::value_type(genomeLocationOfThisHit, blah));
                } else {
        ...
        if (searchRadius == 0 || searchRC == true) {
            ...
            for (unsigned i = 0 ; i < min(nRCHits, maxHitsToConsider); i++) {         
                ...
                if ((pos == mapRC.end())) {
                    ...
                    mapRC.insert(seed_map::value_type(genomeLocationOfThisHit, blah));
                } else {
                    pos->second.insert(nextSeedToTest);
                }
    ...
    AlignmentResult
SpecialAligner::AlignRead(Read *read, unsigned *genomeLocation, bool *hitIsRC, int *finalScore)
{
    return AlignRead(read, genomeLocation, hitIsRC, finalScore, 0, 0, false, 0, NULL, NULL, NULL, NULL);
}

    AlignmentResult
SpecialAligner::AlignRead(
    Read      *read,
...
--*/
{
    ...
    *hitIsRC = false;             // So we deterministically print the read forward in this case.
    ...
    while (nSeedsApplied + nRCSeedsApplied < maxSeedsToUse) {
        ...
        //Go through all the positions where this read might occur 
    ...
    */
    ...
    /*
    vector<seed_pair> pairs;
    vector<seed_pair> pairsRC;
    
    //Now go through these locations and figure out if they are part of the same gene or not
    for (seed_map::iterator m0 = map.begin(); m0 != map.end(); ++m0) {
    
        const Genome::Piece *piece0 = genome->getPieceAtLocation(*m0);
        const char* chr0 = piece0->name;
        unsigned pos0 = (*m0) - piece0->beginningOffset + 1;
  
        for (seed_map::iterator m1 = m0; m1 != map.end(); ++m1) {
        
            //Do not compare iterator with itself
            if (m0 == m1) {
                continue;
            }
            pairs.push_back(seed_pair(*m0, *m1));
               
        } 
    }
    
    //Now go through these locations and figure out if they are part of the same gene or not
    for (seed_map::iterator m0 = mapRC.begin(); m0 != mapRC.end(); ++m0) {
    
        const Genome::Piece *piece0 = genome->getPieceAtLocation(*m0);
        const char* chr0 = piece0->name;
        unsigned pos0 = (*m0) - piece0->beginningOffset + 1;
  
        for (seed_map::iterator m1 = m0; m1 != mapRC.end(); ++m1) {
        
            //Do not compare iterator with itself
            if (m0 == m1) {
                continue;
            }
            pairsRC.push_back(seed_pair(*m0, *m1));
            
        }    
    }
    
    if (pairs.size() == 1) {
        for (vector<seed_pair>::iterator it = pairs.begin(); it != pairs.end(); ++it) {
    
            const Genome::Piece *piece0 = genome->getPieceAtLocation((*it).first);
            const char* chr0 = piece0->name;
            unsigned pos0 = ((*it).first) - piece0->beginningOffset + 1; 
        
            const Genome::Piece *piece1 = genome->getPieceAtLocation((*it).second);
            const char* chr1 = piece1->name;
            unsigned pos1 = ((*it).second) - piece1->beginningOffset + 1;   
    
            unsigned distance = abs((int)pos0-(int)pos1);
    
            if ((strcmp(chr0, chr1) == 0) && (distance < 10000))
                printf("PAIR: %s %d %s %d\n", chr0, pos0, chr1, pos1);
        }
    }
    ...
    if (pairsRC.size() == 1) {
        ...
        for (vector<seed_pair>::iterator it = pairsRC.begin(); it != pairsRC.end(); ++it) {
    ...
    */
    ...
    /*
    const Genome::Piece *piece1 = genome->getPieceAtLocation(*m1);
    const char* chr1 = piece1->name;
    unsigned pos1 = (*m1) - piece1->beginningOffset + 1;   
                //Query the GTF to see if we have something within the same gene
            printf("RC PAIR: %s %d %s %d\n", chr0, pos0, chr1, pos1);
    */
    ...
    while (nSeedsApplied + nRCSeedsApplied < maxSeedsToUse) {
        ...
        printf("nextSeedToTest: %d maxSeedsToUse: %d\n", nextSeedToTest, maxSeedsToUse);
        printf("HITS\n");
...
Routine Description:
    ...
    Make progress in scoring a possibly partial alignment.  This is a private method of the SpecialAligner class that's used
    only by AlignRead.
        ...
        if (elementToScore->lowestPossibleScore <= scoreLimit) {
            ...
            while (_BitScanForward64(&candidateIndexToScore,candidatesMask)) {
                ...
                if (elementToScore->isRC) {
                    ...
                    if ((data==NULL) && (amountExceeded <= MAX_K)) {
            ...
            //const char* pieceName = piece->name;
            //unsigned positionInPiece = genomeLocation - piece->beginningOffset + 1; // SAM is 1-based
            //printf("%s %u %u\n", pieceName, positionInPiece, score);
            //////////
...
--*/
{
    ...
    void
SpecialAligner::allocateNewCandidate(unsigned genomeLocation, bool isRC, unsigned lowestPossibleScore, Candidate **candidate, HashTableElement **hashTableElement)
/*++
...
#if     DBG
    bool
SpecialAligner::AssertCandidatesAreSorted()
/*++
...
--*/
{
    ...
    void
SpecialAligner::ComputeHitDistribution(
        Read        *read,
...
SpecialAligner::HashTableElement::HashTableElement()
{
    init();
}

    void
SpecialAligner::HashTableElement::init() 
{
    weightNext = NULL;
    weightPrev = NULL;
    next = NULL;
...
SpecialAligner::Candidate::init()
{
    ...
    void 
SpecialAligner::clearCandidates() {
    hashTableEpoch++;
    nUsedHashTableElements = 0;
    ...
    void
SpecialAligner::incrementWeight(HashTableElement *element)
{
