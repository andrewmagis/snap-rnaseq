...
RNASeqPairedEndAligner::RNASeqPairedEndAligner(
 ...
 :   index(index_), transcriptome(transcriptome_), gtf(gtf_), maxReadSize(maxReadSize_), confDiff(confDiff_), maxHits(maxHits_),
     maxK(maxK_), maxSeeds(maxSeeds_), minSpacing(minSpacing_), maxSpacing(maxSpacing_), maxDist(maxDist_), explorePopularSeeds(explorePopularSeeds_),
     ...
     lv(2 * MAX_BUCKETS)
{
    ...
    specialAligner = new SpecialAligner(gtf, index, confDiff + 1, maxHits, maxK, maxReadSize,
                                    maxSeeds, 1000000 /* lvLimit */, adaptiveConfDiffThreshold);
