...
SAMWriter::generateSAMText(
                ...
                )
 {
    ...
    int charsInString = snprintf(buffer, bufferSpace, "%.*s\t%d\t%s\t%u\t%d\t%s\t%s\t%u\t%lld\t%.*s\t%.*s\tPG:Z:SNAP\tRG:Z:FASTQ%s%s\n",
        qnameLen, read->getId(),
        flags,
        pieceName,
        positionInPiece,
...
bool SimpleSAMWriter::write(Read *read, AlignmentResult result, unsigned genomeLocation, bool isRC, bool isTranscriptome, char flag)
{
    if (file == NULL) {
        return false;
    }

    write(read, result, genomeLocation, isRC, isTranscriptome, flag, false, false, NULL, NotFound, 0, false, false, 0);
    return true;
}
...
bool SimpleSAMWriter::writePair(Read *read0, Read *read1, PairedAlignmentResult *result)
{
    ...
    if (result->location[0] > result->location[1]) {
        write(read1, result->status[1], result->location[1], result->isRC[1], result->isTranscriptome[1], result->flag[1], true, true,
              read0, result->status[0], result->location[0], result->isRC[0], result->isTranscriptome[0], result->flag[0]);
        write(read0, result->status[0], result->location[0], result->isRC[0], result->isTranscriptome[0], result->flag[0], true, false,
              read1, result->status[1], result->location[1], result->isRC[1], result->isTranscriptome[1], result->flag[1]);
    } else {
        write(read0, result->status[0], result->location[0], result->isRC[0], result->isTranscriptome[0], result->flag[0], true, true,
              read1, result->status[1], result->location[1], result->isRC[1], result->isTranscriptome[1], result->flag[1]);
        write(read1, result->status[1], result->location[1], result->isRC[1], result->isTranscriptome[1], result->flag[1], true, false,
              read0, result->status[0], result->location[0], result->isRC[0], result->isTranscriptome[0], result->flag[0]);
    }
    return true;
}
        ...
        char mateFlag)
{
    ...
    if (!generateSAMText(read, result, genomeLocation, isRC, isTranscriptome, flag, useM, hasMate, firstInPair, mate, mateResult,
            mateLocation, mateIsRC, mateIsTranscriptome, mateFlag, genome, transcriptome, gtf, lv, bsd, outputBuffer, maxLineLength,&outputBufferUsed)) {
...
ThreadSAMWriter::close()
{
    ...
    bool
ThreadSAMWriter::write(Read *read, AlignmentResult result, unsigned genomeLocation, bool isRC, bool isTranscriptome, char flag)
{
    size_t sizeUsed;
    if (!generateSAMText(read, result, genomeLocation, isRC, isTranscriptome, flag, useM, false, true, NULL, UnknownAlignment, 0, false, false, 0, genome, transcriptome, gtf, lv, bsd,
...
ThreadSAMWriter::writePair(Read *read0, Read *read1, PairedAlignmentResult *result)
{
    ...
    bool writesFit = generateSAMText(reads[first], result->status[first], result->location[first], result->isRC[first], result->isTranscriptome[first], result->flag[first], useM, true, true,
                                     reads[second], result->status[second], result->location[second], result->isRC[second], result->isTranscriptome[second], result->flag[second],
    ...
    if (writesFit) {
        writesFit = generateSAMText(reads[second], result->status[second], result->location[second], result->isRC[second], result->isTranscriptome[second], result->flag[second], useM, true, false,
                                    reads[first], result->status[first], result->location[first], result->isRC[first], result->isTranscriptome[first], result->flag[first],
    ...
    if (!writesFit) {
        ...
        if (!generateSAMText(reads[first], result->status[first], result->location[first], result->isRC[first], result->isTranscriptome[first], result->flag[first], useM, true, true,
                             reads[second], result->status[second], result->location[second], result->isRC[second], result->isTranscriptome[second], result->flag[second],
                             genome, transcriptome, gtf, lv, bsd, buffer[bufferBeingCreated] + bufferSize - remainingBufferSpace,remainingBufferSpace,&sizeUsed[first],
                             idLengths[first]) ||
            !generateSAMText(reads[second], result->status[second] ,result->location[second], result->isRC[second], result->isTranscriptome[second], result->flag[second], useM, true, false,
                             reads[first], result->status[first], result->location[first], result->isRC[first], result->isTranscriptome[first], result->flag[first],
                             genome, transcriptome, gtf, lv, bsd, buffer[bufferBeingCreated] + bufferSize - remainingBufferSpace + sizeUsed[first],remainingBufferSpace-sizeUsed[first],&sizeUsed[second],
                             idLengths[second])) {
