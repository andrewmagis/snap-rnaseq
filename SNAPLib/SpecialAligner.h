...
class SpecialAligner: public Aligner {
public:
    ...
    virtual ~SpecialAligner();
    
    AlignmentResult CharacterizeSeeds(Read *read, unsigned searchRadius, unsigned searchLocation, bool searchRC, seed_map &map, seed_map &mapRC);
...
private:
    ...
    // SpecialAligner object, and if the two differ then the hash table bucket is empty.  We increment
    // the epoch number in the SpecialAligner at the beginning of each alignment, thus effectively
    ...
    inline unsigned getWrappedNextSeedToTest(unsigned wrapCount) {
            ...
            switch (seedLen) {
                ...
                default: fprintf(stderr,"SpecialAligner: Not set up to run with this seed size\n"); exit(1);
            } // outer switch
        } 
};
